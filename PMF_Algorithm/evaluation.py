...
def figure(values_list, name='', color=''):
    plt.rcParams['font.sans-serif'] = ['Songti SC']
    plt.rcParams['font.size'] = 10.5
    x = range(len(values_list))
    plot(x, values_list, color=color, linewidth=5)
    plt.title(name + ' curve', fontdict={'family': 'sans-serif'})
    plt.xlabel('Iterations', fontdict={'family': 'sans-serif'})
    plt.ylabel(name, fontdict={'family': 'sans-serif'})
    plt.show()
